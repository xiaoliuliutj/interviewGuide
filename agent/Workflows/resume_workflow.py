...
from agent.Common.Exceptions.agent_exception import AgentRequestContractError, RagDocumentParseError
from agent.Common.results import AgentError, AgentOperationResponse, AgentResultStatus
...
from agent.Workflows.resume_models import ResumeEvaluation, ResumeJobStatus
...
class ResumeWorkflow:
    ...
    async def handleRequest(self, request: AgentOperationRequest) -> AgentOperationResponse:
        ...
        jobRunId = self.readOptionalString(payload, "resumeRunId")
        ...
        resumeId = self.readOptionalString(payload, "resumeId")
    ...
    async def upload(self, request: AgentOperationRequest) -> AgentOperationResponse:
        ...
        result = await self.repository.saveUpload(
            ...
            self.readOptionalString(payload, "contentType"),
            content,
            self.readOptionalString(payload, "targetRole"),
    ...
    async def evaluate(self, resumeId: str, text: str, targetRole: str) -> ResumeEvaluation:
        ...
        messages = [
            {"role": "system", "content": self.promptLoader.loadPrompt("Resume/analysis.txt")},
            {
                "role": "user",
                "content": json.dumps({
                    "resumeId": resumeId,
    ...
    def readOptionalString(self, payload: dict[str, object], field: str) -> str | None:
        """读取可选简历元数据，空值统一归一为 None。"""
        value = payload.get(field)
