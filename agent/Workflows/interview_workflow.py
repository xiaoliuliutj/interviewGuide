...
from agent.Agents.models import AgentContext
...
from agent.Common.results import AgentError, AgentOperationResponse, AgentResultStatus, AgentTaskType
...
class InterviewWorkflow:
    ...
    async def startInterview(self, request: AgentOperationRequest) -> AgentOperationResponse:
        ...
        try:
            targetRole = self.readOptionalString(payload, "targetRole") or "通用技术岗位"
            difficulty = self.readDifficulty(payload.get("difficulty"))
            plan = await self.createPlan(request, targetRole, difficulty, resumeId)
            opening = self.promptLoader.loadPrompt("Interview/opening.txt", targetRole=targetRole)
