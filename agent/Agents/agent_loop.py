...
from agent.Agents.ports import MemoryGateway, RagGateway, SkillGateway, ToolGateway
...
class AgentLoop:
    """Runs a bounded ReAct loop while delegating all domain capabilities through ports."""
    ...
    def __init__(
        ...
        """Inject capability ports and enforce the maximum number of ReAct reasoning steps."""
        self.llmClient = llmClient
        self.skillGateway = skillGateway
        self.toolGateway = toolGateway
        self.memoryGateway = memoryGateway
        self.ragGateway = ragGateway
    ...
    async def run(self, command: AgentLoopCommand) -> AgentOperationResponse:
        ...
        try:
            ...
            if isConversation:
                replay = await self.memoryGateway.loadRunResult(
                ...
                if replay is not None:
                    return replay
            skill = await self.skillGateway.resolveSkill(command.request.task_type)
            ...
            if command.request.task_type == AgentTaskType.RESUME_MEMORY_DELETION:
                await self.memoryGateway.deleteResumeMemory(
                    command.request.context.principal_id,
            ...
            if command.request.task_type == AgentTaskType.INTERVIEW_MEMORY_DELETION:
                await self.ragGateway.clearSessionCache(command.request.context.conversation_id)
                await self.ragGateway.deleteSessionSources(command.request.context.conversation_id)
                await self.memoryGateway.deleteInterviewMemory(
                    command.request.context.principal_id,
                    command.request.context.conversation_id,
                )
                return self.createNoDataResponse(context)
            ...
            context.messages.append(
                AgentMessage(
                    role="system",
                    content=self.promptLoader.loadPrompt("Agent/response_protocol.txt"),
                )
            )

            if command.request.task_type == AgentTaskType.INTERVIEW_SESSION_INITIALIZATION or isConversation:
                await self.memoryGateway.initializeSession(context)
                candidate = command.parsedPayload.get("candidate")
                if isinstance(candidate, dict):
                    await self.memoryGateway.saveUserProfile(
                        command.request.context.principal_id,
                        candidate,
                    )

            if isConversation:
                turnStatus = await self.memoryGateway.startTurn(context)
            ...
            for step in range(1, self.maxSteps + 1):
                ...
                if isConversation:
                    await self.memoryGateway.renewTurnLease(context)
                ...
                if modelResponse.finalData is not None:
                    ...
                    if command.request.task_type in {
                        ...
                        await self.memoryGateway.saveResumeEvaluation(
                            command.request.context.principal_id,
                            str(command.parsedPayload["resumeId"]),
                    ...
                    if (
                        ...
                        await self.ragGateway.clearSessionCache(command.request.context.conversation_id)
                        await self.memoryGateway.saveInterviewCompletion(
                            command.request.context.principal_id,
                            command.request.context.conversation_id,
                            modelResponse.finalData,
                        )
                    ...
                    if isConversation:
                        response = response.model_copy(
                            update={
                                "state_version": await self.memoryGateway.finishTurn(
                                    context,
                    ...
                    if isConversation:
                        await self.memoryGateway.saveRunResult(response)
                    return response
    ...
    async def cleanupFailedTurn(self, context: AgentContext) -> None:
        ...
        try:
            await self.memoryGateway.abortTurn(context)
        except Exception:
            return
    ...
    async def enrichContext(self, context: AgentContext) -> None:
        ...
        if context.skill.memoryEnabled:
            try:
                memoryItems = await self.memoryGateway.loadMemory(context)
        ...
        if context.skill.ragEnabled:
            try:
                knowledgeItems = await self.ragGateway.retrieveKnowledge(context)
    ...
    async def executeRequestedTool(
        ...
        if toolCall.name not in context.skill.allowedToolNames:
            ...
            try:
                result = await self.toolGateway.executeTool(toolCall, context)
            except Exception as error:
